...
save_current_state() {
    if [[ -n $(git status --porcelain) ]]; then
        ...
        git stash push -m "Temporary stash before running tests"
        return 0
    fi
    return 1
}
...
Co-authored-by: ${COAUTHORS[1]}"
    ...
    # Add description and metadata based on case
    case "$case" in
        1)  # Breaking change in PR description only
            commit_message="$commit_message

Initial commit for testing PR description breaking changes."
            ;;
        2)  # Breaking change in commit only
            commit_message="$commit_message
...
Initial commit for testing PR description co-authors."
            ;;
        5)  # Co-author in commit only
            if [ "$rand_coauthors" = true ]; then
                random_coauthor=$(get_random_element COAUTHORS)
                commit_message="$commit_message
...
main() {
    ...
    had_changes=$(save_current_state)
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            *) echo "Unknown option: $1"; display_help; restore_current_state $had_changes; exit 1 ;;
        esac
        shift
    done
    ...
    # Restore previous state if there were changes
    restore_current_state $had_changes
}
